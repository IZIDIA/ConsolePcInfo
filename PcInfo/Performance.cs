			...
			foreach (IHardware hardware in computer.Hardware) {
				...
				foreach (ISensor sensor in hardware.Sensors) {
					if (sensor.SensorType == SensorType.Load && sensor.Name == "CPU Total") {
						CPULoad.Add(new KeyValuePair<string, string>(string.Format("{0}", sensor.Hardware.Name), string.Format("{0}", sensor.Value)));
					}
				}
			}
		}
			...
			foreach (IHardware hardware in computer.Hardware) {
				...
				foreach (ISensor sensor in hardware.Sensors) {
					if (sensor.SensorType == SensorType.Load && sensor.Name == "GPU Core") {
						GPULoad.Add(new KeyValuePair<string, string>(string.Format("{0}", sensor.Hardware.Name), string.Format("{0}", sensor.Value)));
					}
				}
			}
		}
			...
			foreach (IHardware hardware in computer.Hardware) {
				...
				foreach (ISensor sensor in hardware.Sensors) {
					if (sensor.SensorType == SensorType.Load && sensor.Name == "Memory") {
						RAMLoad.Add(new KeyValuePair<string, string>(string.Format("{0}", sensor.Hardware.Name), string.Format("{0}", sensor.Value)));
					}
				}
			}
		}
